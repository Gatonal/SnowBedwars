...
public final class Arena {
    ...
    private void start() {
        ...
        for (UUID uuid : players) { Player player=Bukkit.getPlayer(uuid); if (player != null) { BedWarsTeam team=getTeam(uuid); if (team != null && team.getSpawn()!=null) player.teleport(team.getSpawn()); plugin.getMessages().send(player, "game-started"); } }
    }
    public void finish() { if (state == ArenaState.ENDING || state == ArenaState.RESTARTING) return; state=ArenaState.ENDING; stateSeconds=plugin.getConfig().getInt("arena.ending-seconds", 10); }
    private void restart() { state=ArenaState.RESTARTING; plugin.getWorldRestorer().restore(this); }
    public void resetComplete() { state=ArenaState.WAITING; stateSeconds=0; gameSeconds=0; players.clear(); stats.clear(); for (BedWarsTeam team:teams.values()) { team.restoreBed(); for (UUID uuid:new ArrayList<UUID>(team.getMembers())) team.remove(uuid); } }
